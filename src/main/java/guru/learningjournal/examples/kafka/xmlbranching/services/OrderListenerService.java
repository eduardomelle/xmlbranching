	...
	public KStream<String, Order>[] process(KStream<String, String> input) {
		...
		KStream<String, OrderEnvelop> orderEnvelopKStream = input.map((key, value) -> {
			...
			try {
				...
				Order order = (Order) jaxbUnmarshaller.unmarshal(new StringReader(value));

				orderEnvelop.setValidOrder(order);
		...
		orderEnvelopKStream.filter((k, v) -> !k.equalsIgnoreCase(AppConstants.VALID_ORDER)).to(ERROR_TOPIC,
				Produced.with(AppSerdes.String(), AppSerdes.orderEnvelop()));
